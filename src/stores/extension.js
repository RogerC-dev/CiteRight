...
export const useExtensionStore = defineStore('extension', () => {
  ...
  function openDictionaryPanel() {
    console.log('📖 開啟辭典面板 - 來自擴充功能彈出視窗')

    import('./sidebar.js').then(({ useSidebarStore }) => {
      const sidebarStore = useSidebarStore()
      sidebarStore.open()
      sidebarStore.setCurrentTab('dictionary')
      console.log('✅ 辭典面板已開啟')
    }).catch(error => {
      console.error('❌ 開啟辭典面板失敗:', error)
    })
  }

  function openChatPanel() {
    console.log('🤖 開啟 AI 助手面板 - 來自擴充功能彈出視窗')

    import('./sidebar.js').then(({ useSidebarStore }) => {
      const sidebarStore = useSidebarStore()
      sidebarStore.open()
      sidebarStore.setCurrentTab('chat')
      console.log('✅ AI 助手面板已開啟')
    }).catch(error => {
      console.error('❌ 開啟 AI 助手面板失敗:', error)
    })
  }
